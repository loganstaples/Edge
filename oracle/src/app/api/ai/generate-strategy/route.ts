...
- ai_analyst: Config: { instruction: string, model: "claude-haiku"|"claude-sonnet"|"claude-opus", depth: "fast"|"balanced"|"thorough", structured: boolean }. Outputs: analyst_probability (0-1), analyst_confidence, analyst_direction, analyst_reasoning, price (of matched market), analyst_market_title. NOTE: The AI analyst can dynamically search Polymarket for relevant markets during analysis — it is NOT limited to the pre-fetched available_markets list. It will search for markets matching the input data if needed.
...
- For "when event X happens → find relevant market Y" strategies, prefer: standalone event source → ai_analyst (outputs search_terms) → reactive market feed → edge_calculator. This lets the AI dynamically search for relevant markets based on each event.
- ANTI-PATTERN for event-driven strategies: Do NOT create a standalone market feed (polymarket_feed, gemini_markets_feed) that connects directly to edge_calculator when the strategy starts from an event source. WRONG: event_source → ai_analyst → edge_calculator ← market_feed(standalone). CORRECT: event_source → ai_analyst → market_feed(reactive) → edge_calculator. The market feed MUST be downstream of ai_analyst so it receives search_terms and becomes a reactive lookup, not a static entry point.
...
- router: Use for CONDITIONAL branching on any field ("if change_pct > 3", "if sentiment > 50"). Use this for percentage-based conditions.
...
- To achieve this, connect both the AI analysis chain AND a market feed to the edge_calculator so both probability and price data are available.
- Always include a cooldown_gate between edge_calculator and trade_advanced.

Data flow — ai_analyst:
- When the strategy reacts to external events (news, tweets) affecting prediction markets, the ai_analyst needs BOTH the event data AND market data to produce a meaningful probability. Connect both the trigger source and a context market feed to the ai_analyst's input so it can correlate events with specific markets.
- ai_analyst instructions MUST be adaptive to any input — never hardcode assumptions about what the input will be. The instruction should follow this pattern:
  1. ASSESS what the incoming data actually is (not assume "breaking news" or any specific scenario)
  2. SCAN the available market data for any that could be affected
  3. DETERMINE if there's a meaningful connection — if not, output probability 0.5 with confidence "low" so edge_calculator will gate the signal
  4. Only if relevant: ESTIMATE the true probability and direction
- BAD instruction: "A breaking world event has just occurred. Assess its impact on prediction markets." (assumes specific input type, always assumes relevance)
- GOOD instruction: "Examine the incoming data. Check if any of the available prediction markets would be meaningfully affected. If you find a relevant market, estimate the true probability of its outcome given this new information, and compare it to the market's current price. If no market is relevant, set probability to 0.5 and confidence to low."
- The instruction should NEVER presume the input type — it should inspect and react to whatever data arrives.
...
FIELD COMPATIBILITY — nodes read specific named fields from the river. Mismatched fields cause silent failures:
- consensus reads: analyst_probability, consensus_probability, calibrated_probability, formula_result, scanner_sentiment_score. It ignores all other fields.
- edge_calculator reads probability from: analyst_probability, consensus_probability. Reads price from: price, contract_price, yes_price, current_price. If EITHER is missing, the gate blocks.
- sentiment_scanner reads text from: headline, tweet_text, event_title, description. Returns score 0 if no text field found.
- cooldown_gate reads direction from: ec_direction, analyst_direction, direction (for reset_on_reversal).
- price_alert_decide reads price from: price, contract_price, yes_price, current_price.
- When using formula to produce a probability for consensus, set the formula to output a value between 0 and 1 (e.g., "(scanner_sentiment_score + 100) / 200"). The formula_result field is read by consensus.
- When using formula to produce a probability for edge_calculator, this will NOT work directly — edge_calculator does not read formula_result. Instead, pipe formula through consensus (even with 1 input) to produce consensus_probability, which edge_calculator does read.
...
Event-driven → dynamic market search (PREFERRED for event-driven strategies):
  news_monitor (standalone, entry point) → ai_analyst (outputs search_terms + probability + price) → polymarket_feed (reactive, uses search_terms) → edge_calculator → cooldown_gate → trade_advanced + alert_advanced
  The AI analyst examines the news, determines relevant search terms, estimates probability, and the reactive polymarket_feed searches for matching markets using those terms.
  AI analyst instruction: "Examine the news. Identify what prediction markets could be affected. Output search_terms with keywords to find relevant markets. If you can estimate a probability, do so. If no market is relevant, set confidence to low."

Market-monitoring (react to price movements):
  polymarket_feed (standalone) → price_alert_decide → ai_analyst → edge_calculator → cooldown_gate → trade_advanced

Cross-platform arbitrage:
  polymarket_feed (standalone) → arb_detector ← gemini_markets_feed (standalone)
  arb_detector → cooldown_gate → trade_advanced

Chained data sources (any source can feed another):
  twitter_monitor → ai_analyst → news_monitor (reactive, AI-derived keywords) → edge_calculator → trade_advanced
  crypto_price → formula → router → polymarket_feed (reactive on one route) / alert_advanced (on another route)
 ...
 */
function enforceStrategyRules(strategy: { nodes: any[]; connections: any[] }, userPrompt: string) {
  ...
  const dataNodes = strategy.nodes.filter((n: any) => n.category === "data");
  const marketFeeds = dataNodes.filter((n: any) => MARKET_FEED_TYPES.has(n.type));
  const eventSources = dataNodes.filter((n: any) => EVENT_SOURCE_TYPES.has(n.type));
